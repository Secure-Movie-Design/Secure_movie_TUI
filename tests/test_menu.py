...
from movie.menu import MenuDescription, Key, Entry, Menu
...
def test_menu_builder_cannot_create_empty_menu():
    menu_builder = Menu.Builder(MenuDescription('a MenuDescription'))
    with pytest.raises(ValidationError):
        menu_builder.build()
...
def test_menu_selection_call_on_selected(mocked_print, mocked_input):
    menu = Menu.Builder(MenuDescription('a MenuDescription'))\

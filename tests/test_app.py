...
from movie.domain import MovieDealer, Title, Movie, Description, Year, Director, Category, Id, ImageUrl
...
@pytest.fixture
def movie():
    return {"id": 1, "title": "A title", "description": "A description", "year": 2020, "category": "ACTION",
            "director": "A director", "image_url": "https://image.tmdb.org/t/p/w500/eQ4GRmP0EEkxjwlPbZlVn7HLoZp.jpg"}
...
def movies():
    return [Movie(Id(1), Title('A title'), Description('A description'), Year(2020),
            ...
            Movie(Id(2), Title('B title'), Description('A description'), Year(2020),
                  Category(Category.MovieCategory.ACTION), Director('A director')),
            Movie(Id(3), Title('C title'), Description('A description'), Year(2020),
                  Category(Category.MovieCategory.ACTION), Director('A director'))]
...
def test_update_movie_prints_correctly_when_movie_not_found(mock_print, mock_input, app, movie):
    with patch('getpass.getpass', side_effect=['Password43210wewe?']) as password:
        with patch.object(MovieDealer, 'login', return_value="token") as login:
            with patch.object(MovieDealer, 'is_admin_user', return_value=True) as is_admin_user:
                with patch.object(App, '_App__list_movies', return_value=None):
                    with patch.object(App, '_App__read_from_input', return_value=Id(movie['id'])):
...
def test_show_movies__sorted_by_title_prints_correctly(mock_print, mock_input, app, movies):
    with patch.object(MovieDealer, 'sort_movies_by_title', return_value=movies) as get_movies:
        app.run()
